...
(function codeSnippets() {
    ...
    hljs.registerLanguage('vif', function(hljs) {
        const KEYWORDS = [
            'fn', 'if', 'in', 'or', 'and', 'pub', 'var', 'for', 'try',
            'else', 'loop', 'enum', 'test', 'const', 'comptime', 'while',
            'break', 'defer', 'catch', 'union', 'error', 'switch', 'return',
            'struct', 'extern', 'concept', 'requires', 'continue', 'errdefer',
            'undefined', 'unreachable', 'Self', 'self', 'null', 'true', 'false',
        ].join(' ');
        const TYPES = [
            'usize', 'isize', 'bool', 'void', 'never', 'any', 'anyint',
            'anyfloat', 'anyerror', 'type', 'f16', 'f32', 'f64', 'f128',
            'i1', 'i2', 'i3', 'i4', 'i5', 'i6', 'i7', 'i8', 'i9', 'i16',
            'i32', 'i64', 'i128', 'u1', 'u2', 'u3', 'u4', 'u5', 'u6',
            'u7', 'u8', 'u9', 'u16', 'u32', 'u64', 'u128',
        ].join(' ');
        ...
        return {
            ...
            contains: [
                ...
                {
                    className: 'number',
                    begin: /\b(?:0x[0-9A-Fa-f_]+|0b[01_]+|0o[0-7_]+|[0-9][0-9_]*(?:\.[0-9_]+)?)(?:[iu][0-9]+|f(?:16|32|64|128))?\b/,
                },
            ],
        };
    });
...
(function themes() {
    ...
    function set_theme(theme, store = true) {
        ...
        if (theme === 'coal') {
            if (stylesheets.ayuHighlight) stylesheets.ayuHighlight.disabled = true;
            if (stylesheets.tomorrowNight) stylesheets.tomorrowNight.disabled = false;
            if (stylesheets.highlight) stylesheets.highlight.disabled = true;

            ace_theme = 'ace/theme/tomorrow_night';
        } else {
            if (stylesheets.ayuHighlight) stylesheets.ayuHighlight.disabled = true;
            if (stylesheets.tomorrowNight) stylesheets.tomorrowNight.disabled = true;
            if (stylesheets.highlight) stylesheets.highlight.disabled = false;
...
(function clipboard() {
    ...
    Array.from(clipButtons).forEach(function(clipButton) {
        clipButton.addEventListener('click', async function(e) {
            ...
            try {
                const playground = e.currentTarget.closest('pre');
                await navigator.clipboard.writeText(playground_text(playground, false));
                showTooltip(e.currentTarget, 'Copied!');
            } catch {
                showTooltip(e.currentTarget, 'Clipboard error!');
            }
        }, true);
